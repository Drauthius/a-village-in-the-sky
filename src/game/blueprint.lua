...
function Blueprint:createPlacingTile(type)
	...
	tile:add(SpriteComponent(spriteSheet:getSprite(type .. "-tile")))
...
function Blueprint:createPlacingBuilding(type)
	...
	local sprite = spriteSheet:getSprite(type .. (type == "field" and "" or " 0"))
	local collision = spriteSheet:getSprite(type .. " (Grid information)" .. (type == "field" and "" or " 0"))

	building:add(PlacingComponent(false, type))
	building:add(CollisionComponent(collision))
	building:add(SpriteComponent(sprite))
	building:add(WorkComponent(WorkComponent.BUILDER))
